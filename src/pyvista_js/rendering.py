...
class _BaseHTMLRenderer:
    ...
    def _generate_typescript(self) -> str:
        """Generate TypeScript code for vtk.js rendering with type annotations.

        Returns
        -------
        str
            TypeScript code with type annotations for the vtk.js visualization.
        ...
        # Add TypeScript type imports and annotations
        ts_code = f"""// TypeScript code generated by pyvista-js
// vtk.js type definitions
import vtk from '@kitware/vtk.js';
...
function renderScene(containerId: string): void {{
  ...
  const fullScreenRenderer: vtkFullScreenRenderWindow = vtk.Rendering.Misc.vtkFullScreenRenderWindow.newInstance({{
    container: container,
...
{self._generate_lights_code()}

{actors_code}

{self._generate_scalar_bar_code()}

{self._generate_environment_code()}

{self._generate_axes_code()}
