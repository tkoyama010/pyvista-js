...
def generate_torus_point_cloud(
    ...
    x = (radius_major + r * np.cos(theta)) * np.cos(phi)
    y = (radius_major + r * np.cos(theta)) * np.sin(phi)
    z = r * np.sin(theta)
...
def generate_bunny_point_cloud(n_points: int = 5000) -> tuple:
    ...
    # Generate ears as elongated ellipsoids
...
This demonstration showcases pyvista-js's capabilities in scientific
visualization and computer vision domains.
The interactive 3D point cloud viewer provides features similar to the
VGGT (Visual Geometry Grounded Transformer) demo, which was the CVPR 2025
Best Paper Award winner for 3D reconstruction from images.
""")
